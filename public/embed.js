...
(function () {
  ...
  css.textContent =
    '#__sa-bubble{position:fixed;bottom:24px;' + side + ':24px;z-index:2147483647;' +
    ...
    '#__sa-bubble:hover{transform:scale(1.1) translateY(-2px);box-shadow:0 12px 32px ' + color + '55,0 4px 12px rgba(0,0,0,.15)}' +
    '#__sa-bubble:active{transform:scale(.95)}' +
    '#__sa-bubble svg{width:28px;height:28px;fill:#fff;transition:transform .3s}' +
    '#__sa-frame{position:fixed;bottom:100px;' + side + ':24px;z-index:2147483647;' +
    ...
    '#__sa-frame.open{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}' +
    '@media(max-width:480px){#__sa-frame{width:100vw;height:100vh;max-height:100vh;' +
    'bottom:0;' + side + ':0;border-radius:0}}';
  document.head.appendChild(css);

  // ─── Bubble ──────────────────────────────────────────────────────────────
  var bubble = document.createElement('button');
  bubble.id = '__sa-bubble';
